...
av_size = features[features['ROI_type'] == 'knob'].groupby(['image_name', 'cell_number']).mean()['area_nm'].reset_index()

# --------------Calculate proportion of area in 'knobs'--------------
cell_size = features[features['ROI_type'] == 'cell'].set_index(['image_name', 'cell_number'])['area_nm']
knob_area = features[features['ROI_type'] == 'knob'].groupby(['image_name', 'cell_number']).sum()['area_nm']
knob_proportion = ((knob_area / cell_size)*100).reset_index().rename(columns={'area_nm': 'proportion_knob_area'})
...
knob_count = features[features['ROI_type'] == 'knob'].groupby(['image_name', 'cell_number']).count()['area_nm'] 
knobs_per_area = (knob_count / (cell_size/10000)).reset_index().rename(columns={'area_nm': 'knobs_per_100_nm2'})

# --------------Summarise, save to csv--------------
summary = functools.reduce(lambda left, right: pd.merge(left, right, on=['image_name', 'cell_number'], how='outer'), [av_size, cell_size.reset_index(), knob_area.reset_index(), knob_proportion, knob_count.reset_index(), knobs_per_area, minor_axis, major_axis])
summary.columns = ['image_name', 'cell_number', 'mean_knob_area', 'cell_size', 'total_knob_area',
                   'knob_area_proportion', 'knob_count', 'knobs_per_100_nm2', 'mean_minor_axis', 'mean_major_axis']
