...
input_folder = f'SEM-internal-cell-texture/python_results/SEM-internal-cell-texture_Analyzed/feature_collection/'
output_folder = f'SEM-internal-cell-texture/python_results/SEM-internal-cell-texture_Analyzed/summary_calculations/'
scale_folder = f'SEM-internal-cell-texture/python_results/SEM-internal-cell-texture_Analyzed/napari_scale/'
...
features['area_nm'] = (features['area'] / features['pixels_nm'])
...
minor_axis = features[features['ROI_type'] == 'knob'].groupby(
    ['image_name']).mean()['minor_axis_length_nm']
major_axis = features[features['ROI_type'] == 'knob'].groupby(
    ['image_name']).mean()['major_axis_length_nm']

# ----------Calculate average size of knobs per cell----------
av_size = features[features['ROI_type'] == 'knob'].groupby(
    ['image_name']).mean()['area_nm'].reset_index()

# -----------Calculate proportion of area in 'knobs'-----------
knob_area = features[features['ROI_type'] == 'knob'].groupby(['image_name']).sum()['area_nm']
...
knob_count = features[features['ROI_type'] == 'knob'].groupby(['image_name']).count()['area_nm'] 
knobs_per_area = (knob_count / (10000)).reset_index().rename(columns={'area_nm': 'knobs_per_100_nm2'})

# ------------------Summarise, save to csv------------------
summary = functools.reduce(lambda left, right: pd.merge(left, right, on=['image_name'], how='outer'), [av_size, knob_area.reset_index(), knob_count.reset_index(), knobs_per_area, minor_axis, major_axis])
summary.columns = ['image_name', 'mean_knob_area', 'total_knob_area', 'knob_count', 'knobs_per_100_nm2', 'mean_minor_axis', 'mean_major_axis']
...
for parameter in ['mean_knob_area', 'knob_count', 'knobs_per_100_nm2', 'mean_minor_axis', 'mean_major_axis']:
    fig, ax = plt.subplots()
    sns.swarmplot(x='cell_line', y=parameter,
                  data=summary, palette=palette_ko)
    plt.xlabel('cell_line')
    plt.ylabel(parameter)
    plt.show()

sns.histplot(x=summary['mean_major_axis'],
             kde=True, hue=summary['cell_line'], palette=palette_ko)
plt.show()
